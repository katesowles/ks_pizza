...
function createShop(loc, address, cityState, pieRange, delRange, dailyTotal, pizzaResult, shopTotal, pizzas) {
  this.loc = loc;
  ...
  this.pizzas = function(pieRange) { // shopLocation = shops[x]
      ...
      var max;
        for (var y = 0; y < timeBlocks.length; y++) {
          ...
          this.pizzaResult.push(randomizer);
          // console.log("pizza result: " + pizzaResult);
          this.shopTotal = this.shopTotal + randomizer;
...
var deliveries = function(shopLocation) {
    //pizzas(shopLocation);
    console.log("Actual Shop: " + shopLocation);
    console.log("shopTotal: " + shopTotal);
    ...
    //   console.log("HERE: " + deliveryResult);
      ...
      else if (shopLocation.delRange[repeatByTimeBlock][1] > shopTotal) { // if the max number of deliveries is higher (but not equal to) than the number of pizzas baked, cap the number of deliveries at # pizzas.
      ...
      var tblHead = document.createElement("thead"); // creates <thead> and puts it in <table>
        ...
          shopLoc.setAttribute("colspan", "4"); // sets the colspan attribute of tbl to 4;
          shopHead.appendChild(shopLoc);
            ...
            var shopLocation = document.createTextNode(shops[shopIndex].loc); // pulls the text for the shopLoc <th> from the  universal.shops[i].locations array
        ...
        var colHead = document.createElement("tr"); // creates the colHead <tr> and puts it in <thead>
          ...
          var tblBody = document.createElement("tbody");
            ...
            for (repeatByTimeBlock = 0; repeatByTimeBlock < timeBlocks.length; repeatByTimeBlock++) { // creates as many rows as there are timeBlocks
              ...
              var numDriversText = document.createTextNode(drivers(shops[shopIndex])); // this has to be called here so that all sections that follow have numbers generate for them to fill in.
...
for (shopIndex = 0; shopIndex < shops.length; shopIndex++) {
    generate_table(shopIndex);
}
